...
const UserLogin: React.FC = () => {
  ...
  const home = useCallback((e: React.MouseEvent) => {
    e.preventDefault();
    navigate("/");
  }, [navigate]);
  ...
  const loginUserData = async (e: React.FormEvent) => {
    e.preventDefault();
    ...
    try {
      ...
      const response: LoginResponse = await dispatch(LoginUser({ email, password })).unwrap();
      
      handleRememberMe();

      const { role, membershipPaymentStatus } = response.landing;
      ...
      if (membershipPaymentStatus === 'not_started') {
        targetPath = '/set-payment-plan';
      } else if (membershipPaymentStatus === 'in-progress') {
        targetPath = '/set-payment-plan/step2';
      }
  ...
  useEffect(() => {
    ...
    const rememberedState = sessionStorage.getItem("rememberMe");

    if (rememberedState === "true" && rememberedEmail && rememberedPassword) {
