...
export function calculateAvailableEndDates(
  ...
  const PRESET_WEEKLY_INTERVALS = config?.weeklyIntervals || [
    1, 2, 4, 8, 12, 24, 52, 104,
  ];
  const PRESET_MONTHLY_INTERVALS =
    config?.monthlyIntervals || Array.from({ length: 24 }, (_, i) => i + 1);
  ...
  if (type === "daily") {
    ...
    PRESET_WEEKLY_INTERVALS.forEach((weeks) => {
      dates.push(formatDate(addDays(startDate, weeks * 7 - 1)));
    });
  } else if (type === "monthly") {
    PRESET_MONTHLY_INTERVALS.forEach((months) => {
      const endDate = addMonths(startDate, months);
...
export function validateCustomEndDate(
  ...
  // Additional validation for weekly type
  if (type === "weekly") {
    ...
    if (diffDays < 7) {
      return {
        isValid: false,
        error: "Weekly contributions must be at least 1 week apart",
      };
    }
  }
